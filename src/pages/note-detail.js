...
import { db } from "../lib/firebase"; // path-nya jadi ../lib/firebase
...
export default function NoteDetail() {
  ...
  const { id } = router.query; // Ambil ?id=xxx dari URL
      ...
      {!editMode ? (
        <>
          ...
          <div className="mb-2">
            <b>Folder:</b>{" "}
            {note.folderId
                ? (folders.find(f => f.id === note.folderId)?.name || <span className="text-muted">Tanpa Folder</span>)
